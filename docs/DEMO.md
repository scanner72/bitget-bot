...
Публичный лог для судей (не live mainnet): [`docs/evidence/paper_trading_log.csv`](evidence/paper_trading_log.csv) — `python scripts/export_paper_log.py` из локального `data/paper_fills.jsonl`.  
